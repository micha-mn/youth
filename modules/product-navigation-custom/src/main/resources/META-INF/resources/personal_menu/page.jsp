...
<aui:script require="clay-dropdown/src/ClayDropdown as ClayDropdown,metal-dom/src/dom as dom, metal-position/src/Position">
	...
	var profileTranslated= "<liferay-ui:message key="profile" />";
	console.log("profileTranslated : " + profileTranslated);
    if (toggle) {
		dom.once(
			toggle,
			'click',
			function(event) {
				fetch(
					...
					/*for (var i = 0; i < personalMenuItems.length; i++)
					{
						if (personalMenuItems[i].items[0].href.indexOf('logout') == -1
							&& personalMenuItems[i].items[0].href.indexOf('notifications') == -1)
							//personalMenuItems.splice(i-1,i+1);
					}*/
					var profilePrefs, profileObj = new Object();
						profilePrefs = [{"href":"<%=themeDisplay.getPortalURL() %>/profile", "icon":"", "active":false, label: "<liferay-ui:message key="profile" />"}];
						profileObj.items=profilePrefs;
						profileObj.type= "group";
						console.log(personalMenuItems);
						personalMenuItems.splice(personalMenuItems.length - 2,0,profileObj);
						personalMenuItems.join();
